...
public class Simulator implements Constants
{
	...
	private void createProcess() {
		...
		flushMemoryQueue();
		newProcess.resetTimeToNextIoOperation();
	...
	private void switchProcess() {
		...
		Process p = cpu.removeNext();
		p.setCpuTimeNeeded(p.getCpuTimeNeeded()-maxCpuTime);
		
		if (p != null){
			p.setTimeSpentInCpu(clock);
			p.setCpuTimeNeeded(p.getCpuTimeNeeded()-maxCpuTime);
	...
	private void createEvent(Process p){
		if (p == null)
			return;
		System.out.println(p.getTimeToNextIoOperation());
		System.out.println(p.getCpuTimeNeeded());
		if (p.getTimeToNextIoOperation() > maxCpuTime && p.getCpuTimeNeeded() > maxCpuTime){
	...
	private void evaluateProcess() {
		...
		if(!cpu.isIdle()) {
			...
			if(cpu.getRunningProcess().getTimeToNextIoOperation() < (maxCpuTime)  && cpu.getRunningProcess().getTimeToNextIoOperation() < cpu.getRunningProcess().getCpuTimeNeeded()){
				...
				Process needIO = cpu.removeNext();
				// Updates the objects value for time spent in CPU
	...
	*/
	...
	private void endIoOperation() {
		...
		Process p = io.removeRunning();
		p.setTimeSpentInIo(clock);
		cpu.insertProcess(p);
